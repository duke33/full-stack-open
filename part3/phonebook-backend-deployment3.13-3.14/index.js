...
app.get('/api/persons/:id', (req, res, next) => {
  Person.findById(req.params.id).then(person => {
    res.json(person)
    if (person) { res.json(person) } else {
...
app.get('/info', (req, res) => {
  ...
  Person.countDocuments().then(response => {
    ...
    res.send(`<p>Phonebook has info for: ${response} people</p>\n ${new Date(Date.now())}`)
  })
...
app.delete('/api/persons/:id', (req, res, next) => {
  ...
  Person.findByIdAndDelete(req.params.id).then(personDeleted => {
...
app.put('/api/persons/:id', (req, res, next) => {
  ...
  Person.findByIdAndUpdate(req.params.id, { number: req.body.number }).then(personUpdated => {
...
app.post('/api/persons', (req, res, next) => {
  ...
  //This code was used for validation but has been replaced by the
  //mongoose and mongoose-unique validation
  // if (!(newPerson.name)) {
